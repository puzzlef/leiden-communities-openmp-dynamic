...
void runExperiment(const G& x) {
  ...
  runBatches(x, rnd, [&](const auto& y, auto deletionsf, const auto& deletions, auto insertionsf, const auto& insertions, int sequence, int epoch) {
    ...
    runThreads(epoch, [&](int numThreads) {
      ...
      auto b2 = leidenNaiveDynamicOmp(rnd, y, deletions, insertions, B2, VW, CW, {repeat, resolution, dynamicTolerance});
      flog(b2, "leidenNaiveDynamicOmp");
      // Find delta-screening based dynamic Leiden.
      auto b3 = leidenDynamicDeltaScreeningOmp(rnd, y, deletions, insertions, B3, VW, CW, {repeat, resolution, dynamicTolerance});
      flog(b3, "leidenDynamicDeltaScreeningOmp");
      // Find frontier based dynamic Leiden.
      auto b4 = leidenDynamicFrontierOmp(rnd, y, deletions, insertions, B4, VW, CW, {repeat, resolution, dynamicTolerance});

...
/**
 * Initialize a subset of communities such that each vertex is its own community.
...
inline void leidenInitializeU(vector<K>& vcom, vector<W>& ctot, const G& x, const vector<W>& vtot, FR fr) {
  x.forEachVertexKey([&](auto u) {
    if (!fr(u)) return;
    vcom[u] = u;
    ctot[u] = vtot[u];
 ...
 */
template <class G, class K, class W, class FR>
inline void leidenInitializeOmpU(vector<K>& vcom, vector<W>& ctot, const G& x, const vector<W>& vtot, FR fr) {
  size_t S = x.span();
  #pragma omp parallel for schedule(static, 2048)
  for (K u=0; u<S; ++u) {
    if (!x.hasVertex(u)) continue;
...
inline void leidenInitializeW(vector<K>& vcom, vector<W>& ctot, const G& x, const vector<W>& vtot) {
  ...
  leidenInitializeU(vcom, ctot, x, vtot, fr);
}
...
inline void leidenInitializeOmpW(vector<K>& vcom, vector<W>& ctot, const G& x, const vector<W>& vtot) {
  auto fr = [](auto u) { return true; };
  leidenInitializeOmpU(vcom, ctot, x, vtot, fr);
}
#endif
...
inline int leidenMoveW(vector<K>& vcom, vector<W>& ctot, vector<B>& vaff, vector<K>& vcs, vector<W>& vcout, xorshift32_engine& rng, const G& x, const vector<K>& vcob, const vector<W>& vtot, double M, double R, int L, FC fc, FA fa, FB fb) {
  ...
  for (; l<L;) {
    ...
    x.forEachVertexKey([&](auto u) {
      ...
      if (REFINE) x.forEachEdgeKey(u, [&](auto v) { c = vcom[v]; });
      if (e && c!=d && ctot[d]==vtot[u] && ctot[c]>0.5 && vtot[u]>0.5 && !blocked[vcom[u]]) {
        if (REFINE) n++;
        ctot[d] -= vtot[u];
        ctot[c] += vtot[u];
        vcom[u] = c;
        if (REFINE) blocked[c] = 1;
        if (!REFINE) { fb(d); fb(c); }
    ...
    if (REFINE) printf("Refine phase: moved=%d\n", n);
 ...
 */
template <bool DYNAMIC=false, bool SUBREFINE=false, bool RANDOM=false, bool USEPARENT=false, class FLAG=char, class RND, class G, class FI, class FM, class FA>
    ...
    mark([&]() {
      ...
      for (l=0, p=0; M>0 && P>0;) {
        ...
        tr += measureDuration([&]() {
          ...
          if (isFirst) leidenInitializeU(ucom, ctot, x, utot, fr);
    ...
    mark([&]() {
      ...
      for (l=0, p=0; M>0 && P>0;) {
        ...
        tr += measureDuration([&]() {
          ...
          if (isFirst) m += leidenMoveW<true, RANDOM>(ucom, ctot, vaff, *vcs[0], *vcout[0], *rng[0], x, vcob, utot, M, R, L, fc, fr);
          else         m += leidenMoveW<true, RANDOM>(vcom, ctot, vaff, *vcs[0], *vcout[0], *rng[0], y, vcob, vtot, M, R, L, fc);
          if (!DYNAMIC && isFirst) {
            size_t DC = countValue(communitiesDisconnectedOmp(x, ucom), char(1));
            size_t C  = communities(x, ucom).size();
...
inline auto leidenStatic(RND& rnd, const G& x, const LeidenOptions& o={}) {
  ...
  auto fa = [ ](auto u) { return true; };
  return leidenInvoke<false, false, RANDOM, USEPARENT, FLAG>(rnd, x, o, fi, fm, fa);
}
...
inline auto leidenStaticOmp(RND& rnd, const G& x, const LeidenOptions& o={}) {
  ...
  auto fa = [ ](auto u) { return true; };
  return leidenInvokeOmp<false, false, RANDOM, USEPARENT, FLAG>(rnd, x, o, fi, fm, fa);
}
#endif
#pragma endregion
...
inline auto leidenNaiveDynamic(RND& rnd, const G& y, const vector<tuple<K, K, V>>& deletions, const vector<tuple<K, K, V>>& insertions, const vector<K>& q, const vector<W>& qvtot, const vector<W>& qctot, const LeidenOptions& o={}) {
  ...
  auto fa = [ ](auto u) { return true; };
  return leidenInvoke<true, true, RANDOM, USEPARENT, FLAG>(rnd, y, o, fi, fm, fa);
}
